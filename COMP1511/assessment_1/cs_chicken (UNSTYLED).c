...
int main(void) {
	...
	char mode = setup_phase(board, &player, &target_points, &baby_chicken_total);
...
void print_board(
	...
	for (int row = 0; row < ROWS; row++) {
		for (int col = 0; col < COLS; col++) {
			struct position baby_chicken;
			baby_chicken.row = row;
			baby_chicken.col = col;
			...
			} else if (board[row][col].baby_chicken > 0 && check_baby_chicken_connected(board, baby_chicken, *player) && !(board[row][col].entity == KANGAROO)) {
			...
			} else if (board[row][col].entity == KANGAROO && row == player->row && col == player->col) {
				printf("_B^");
...
void set_wombat(struct tile board[ROWS][COLS], struct position player, int *wombat_count) {
	...
	scanf(" %c %d %d %d %d", &t, &tunnel_one.row, &tunnel_one.col, &tunnel_two.row, &tunnel_two.col);
...
void set_baby_chicken(int *baby_chicken_total) {
	if (*baby_chicken_total < MAX_BABY_CHICKENS) {
		*baby_chicken_total += 1;
		...
		printf("Invalid command: there can be at most %d baby chickens at a time.\n", MAX_BABY_CHICKENS);
	}
}
...
void gameplay_phase(struct tile board[ROWS][COLS], struct position *player, int *target_points, char mode, int baby_chicken_total) {
	...
	while (playing == 1) {
		...
		if (command == 'w' || command == 'a' || command == 's' || command == 'd') {
			player_moved = move_player(board, player, command, &turns_taken, &step_count, mode, &jump_distance, &baby_chicken_count, baby_chicken_total);
		...
		if (playing == 1 && command != 'q' && command != 'p') {
			...
			if (mode == 'd' && !check_is_dead(board, *player)) {
				...
				if (!check_is_dead(board, *player)) {
					scroll_board(board, player, command, &step_count, player_moved, jump_distance);
					has_coin(board, player, &score, &coins);
				}
			}

			print_board(board, player, score, *target_points, baby_chicken_count);
			playing = check_win_loss(board, player, score, *target_points, &playing, turns_taken, step_count, coins);
		}
	}
}
...
int check_in_bounds(struct position position) {
	if ((0 <= position.row) && (position.row < ROWS) && (0 <= position.col) && (position.col < COLS)) {
		return TRUE;
	} else {
		return FALSE;
	}
...
// OUTPUT:
// 		int (1 for true, 0 for false)
int check_touching_tree(struct tile board[ROWS][COLS], struct position position) {
...
int check_is_dead(struct tile board[ROWS][COLS], struct position player) {
	...
	// Baby chicken is touching car
	for (int row = 0; row < ROWS; row++) {
		for (int col = 0; col < COLS; col++) {
			if (
				(board[row][col].baby_chicken > 0 && board[row][col].entity == CAR_FACING_LEFT) ||
				(board[row][col].baby_chicken > 0 && board[row][col].entity == CAR_FACING_RIGHT)
			) {
				return TRUE;
			}
		}
	...
	// Check if there is a lone baby chicken
	for (int row = 0; row < ROWS; row++) {
		for (int col = 0; col < COLS; col++) {
			if (
				...
				for (int i = 0; i < 4; i++) {
					int adj_row = neighours[i].row;
					int adj_col = neighours[i].col;
					
					if (check_in_bounds(neighours[i])) {
						if (board[adj_row][adj_col].baby_chicken > 0) {
							has_adjacent = 1;
						}
					}
				}
...
int check_is_wombat_tunnel(struct tile board[ROWS][COLS], struct position position) {
	if (
		check_in_bounds(position) && 
...
int move_player(
	...
	*jump_distance = 1;
	...
	// Initialise target position for player
	struct position target;
	target.row = player->row;
	target.col = player->col;

	// Initialise offset so the player teleports next to wombat tunnels
	struct position wombat_offset;
	wombat_offset.row = 0;
	wombat_offset.col = 0;

	// If player is on kangaroo, set jump distance
	if (riding_kangaroo) {
	...
	*turns_taken += 1;
	...
	// Kangaroo dismount mechanic
	if (
		check_is_kangaroo(board, *player) &&
		(!check_in_bounds(target) || check_touching_tree(board, target))
	) {
		board[player->row][player->col].entity = board[player->row][player->col].roo_base_entity;
	...
	// Exit function if invalid target
	...
	if (check_is_wombat_tunnel(board, target)) {
		...
		// Check new position and clear baby chickens
...
void move_kangaroo(struct tile board[ROWS][COLS], struct position *player, struct position target) {
	...
	if (!check_is_car(board, target)) {
		...
		board[target.row][target.col].roo_base_entity = board[target.row][target.col].entity;
