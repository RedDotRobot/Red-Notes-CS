...
// on 27-03-2026
//
// User plays as Penny the Chicken, navigating roads and
// outback wildlife
...
void handle_cars(struct tile board[ROWS][COLS]);
void move_cars(
    struct tile board[ROWS][COLS],
...
void initialise_board(struct tile board[ROWS][COLS]) {
    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            ...
            // Clear all coins from board
            // This variable is used to track consumed coin positions
            board[row][col].coin = 0;

            // Set the wombat tunnel id
            // Since 0 is a valid id, set all values to -1
            board[row][col].wombat_id = -1;

            // jump_distance is set depending on whether on a kangaroo
            board[row][col].roo_jump_length = 0;

            // baby_chicken defines whether or not a baby chicken is there
            // Any positive value is a baby chicken
            // On a player turn, the entire board is decremented,
            // creating a chain of baby chickens
            board[row][col].baby_chicken = 0;
        }
    }
}

// Handle printing Penny logic
// INPUT:
// 		board
// 		player position
// 		baby_chicken_count
...
// Handle printing baby chicken logic
// INPUT:
// 		board
// 		position
// OUTPUT:
...
void print_baby_chicken(
    ...
    if (check_is_shocked(board, position)) {
        printf("o.o");
    } else {
        printf("^.^");
    }
...
void print_board(
    ...
    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            struct position tile = {row, col};
            printf("|");
            if (
                check_touching_player(*player, tile) &&
                !(check_is_kangaroo(board, tile))
            ) {
                print_penny(board, player, baby_chicken_count);
            } else if (
                board[row][col].baby_chicken > 0 &&
                check_baby_chicken_connected(board, tile, *player) &&
                !(check_is_kangaroo(board, tile))
            ) {
                print_baby_chicken(board, tile);
...
// 		pointer to int baby_chicken_total
// OUTPUT:
// 		char
char setup_phase(
    struct tile board[ROWS][COLS],
    ...
    scanf(" %c", &command);
    while (command != NORMAL && command != DRIVING && command != SCROLLING) {
        if (command == 'c') {
...
// Prompt user to create pair of WOMBAT_TUNNEL features, modify board if allowed
// INPUT:
// 		board
// 		pointer to player position
// 		pointer to int wombat_count
// OUTPUT:
// 		void
void set_wombat(
    struct tile board[ROWS][COLS],
    ...
    // The wombat tunnel command is two characters
    // The placeholder variable the_letter_t stops scanf reading wrong int
    char the_letter_t;
    ...
    // Validate wombat tunnel positions
...
// Prompt user to create KANGAROO feature, modify board if allowed
// INPUT:
// 		board
// 		pointer to player position
// OUTPUT:
...
// Prompt user to increase baby_chicken_total, increase total if allowed
// INPUT:
// 		pointer to baby_chicken_total
// OUTPUT:
// 		void
void set_baby_chicken(
    int *baby_chicken_total
) {
...
void gameplay_phase(
    ...
    int turns_taken = 0, step_count = 0, coins = 0, score = 0;
    int jump_distance;
    while (playing == TRUE) {
        scanf(" %c", &command);
        int player_moved = 0;
        if (
            ...
            player_moved = move_player(
                board, player, command, &turns_taken,
                &step_count, mode, &jump_distance, &baby_chicken_count,
                baby_chicken_total
            );
        ...
        if (playing == TRUE && command != 'q' && command != 'p') {
            process_game_turn(
                board, player, target_points, mode,
                &turns_taken, &step_count, &coins, &score,
                command, player_moved, jump_distance,
                &baby_chicken_count, baby_chicken_total, &playing
            );
        }
    }
}
...
// 		board
// 		pointer to player position
// 		pointer to target points
// 		mode
// 		pointers to player stats
// 		command
// 		player moved
// 		pointer to playing value
// 		baby chicken values
// OUTPUT:
// 		void
void process_game_turn(
    ...
    if (mode == DRIVING && !check_is_dead(board, *player)) {
        handle_cars(board);
    } else if (mode == SCROLLING && !check_is_dead(board, *player)) {
        handle_cars(board);
        if (!check_is_dead(board, *player)) {
            scroll_board(
                board, player, command,
                step_count, player_moved, jump_distance
            );
            has_coin(board, player, score, coins);
        }
    }
    print_board(
        board, player, *score,
        *target_points, *baby_chicken_count
    );
    check_win_loss(
        board, player, *score, *target_points,
        playing, *turns_taken, *step_count, *coins
    );
}
...
// OUTPUT:
// 		int (TRUE [if player moved] or FALSE)
int handle_wombat_tunnel_teleport(
    struct tile board[ROWS][COLS],
    struct position *player,
    struct position target,
    ...
    // Find the matching wombat tunnel as an exit
    if (find_matching_wombat_tunnel(board, &destination, tunnel_id, target)) {
        ...
        // Make sure the exit position is on the board and not a tree
        if (!check_in_bounds(exit) || check_touching_tree(board, exit)) {
            return FALSE;
        }

        // If it is a valid move, teleport the player and reset baby chickens
...
// Set jump distance if on kangaroo
// INPUT:
// 		board
// 		pointer to player position
// 		player_input
...
// 		baby chicken values
// OUTPUT:
// 		int (TRUE [when riding kangaroo] or FALSE)
int handle_kangaroo_ride(
    struct tile board[ROWS][COLS],
    struct position *player,
    char player_input,
    ...
    // Set default jump distance to 1
    *jump_distance = 1;
    
    // If on a kangaroo, set the jump distance using the tile value
    // Reset baby chickens
    if (riding_kangaroo) {
...
// Calculate the target position for a player movement
// INPUT:
// 		board
// 		pointer to player position
// 		player_input
// 		jump_distance
// 		mode
// 		pointer to wombat_offset (treat as vector rather than position)
// OUTPUT:
// 		struct position
struct position calculate_target_position(
    struct tile board[ROWS][COLS],
    ...
    if (player_input == UP) {
        struct position wombat_check = {ROWS - 1, player.col};
        wombat_offset->row = -1;
...
// Check if kangaroo move is valid and dismount if not
// INPUT:
// 		board
// 		player position
// 		target position
...
int validate_kangaroo_move(
    ...
    if (
        check_is_kangaroo(board, player) &&
...
// 		pointer to player position
// 		target position
//		wombat_offset position (treat as vector)
//		riding_kangaroo (TRUE or FALSE)
//		pointer to step_count
//		baby chicken values
// OUTPUT:
// 		int (TRUE or FALSE)
int execute_move(
...
int move_player(
    ...
    // wombat_offset is changed depending on the direction
    // It ensures that the player is placed next to the exit tunnel
    struct position wombat_offset = {0, 0};
    struct position target = calculate_target_position(
        board, *player,
        player_input,
        *jump_distance,
        riding_kangaroo,
    ...
    *turns_taken += 1;
    ...
    if (!validate_move_and_cleanup(
        board, *player,
        target, riding_kangaroo,
        baby_chicken_count, baby_chicken_total
        )
    ) {
        return FALSE;
...
// 		void
void handle_cars(struct tile board[ROWS][COLS]) {
    // Create a copy of board, don't read board while modifying it
    struct tile next_board[ROWS][COLS];
    buffer_board_no_cars(board, next_board);

    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            struct position position = {row, col};
            move_cars(board, next_board, position);
        }
    }
    buffer_board(next_board, board);
}

// Move cars and handle collisions
// INPUT:
// 		board
// 		next_board
// 		position
...
void move_cars(
    ...
    if (board[position.row][position.col].entity == CAR_FACING_LEFT) {
        int front_col = position.col;
        // Chain together adjacent cars facing the same direction
        while (
            front_col > 0
            && board[position.row][front_col - 1].entity == CAR_FACING_LEFT
        ) {
            front_col--;
        }
        int blocked = !check_is_valid_car_dest(
            board,
            position.row,
            front_col - 1
        );
        // If the chain is blocked, reverse all cars in the chain
        if (blocked) {
            next_board[position.row][position.col].entity = CAR_FACING_RIGHT;
        } else {
            next_board[position.row][position.col - 1].entity = CAR_FACING_LEFT;
        }
    } else if (board[position.row][position.col].entity == CAR_FACING_RIGHT) {
        // Repeat for other direction
        int front_col = position.col;
        while (
            front_col < COLS - 1 &&
            board[position.row][front_col + 1].entity == CAR_FACING_RIGHT
        ) {
            front_col++;
        }
        int blocked = !check_is_valid_car_dest(
        ...
        if (blocked) {
            next_board[position.row][position.col].entity =
                CAR_FACING_LEFT;
        } else {
            next_board[position.row][position.col + 1].entity =
                CAR_FACING_RIGHT;
        }
    }
}

// Scroll the board if in scrolling mode
// INPUT:
// 		board
// 		pointer to player position
// 		player input
...
void scroll_board(
    ...
    if (!(command == UP && player->row < 6)) {
        return;
    }
    //
    // Don't scroll if the player would hit a tree
    if (player->row == 0 && board[ROWS - 1][player->col].entity == TREE) {
        return;
    }

    // Increment step counter
    // Player "doesn't move" while scrolling
    if (!player_moved) {
        *step_count += 1;
    }
    
    // Move the player by jump_distance
    // Ensures that kangaroo in scrolling mode works
    ...
    for (int scroll = 0; scroll < jump_distance; scroll++) {
        ...
        // Copy temp row to top
        // Replace all consumed coins
        for (int col = 0; col < COLS; col++) {
...
// Create a copy of a board without cars
// INPUT:
// 		board (original)
// 		board (copied)
// OUTPUT: void
...
void add_headlights(struct tile board[ROWS][COLS]) {
    ...
    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            ...
            // Initialise headlights position
            // The invalid position is a safety net so that 
            // the position is always defined
            struct position headlights = {-1, -1};
...
// Control kangaroo player movements
// INPUT:
// 		board
// 		pointer to player position
// 		target position
...
// Move all baby chickens by decrementing the baby_chicken
...
// Find the other wombat tunnel given a location and tunnel_id
// INPUT:
// 		board
// 		pointer to destination position
// 		int tunnel_id
// 		target position
// OUTPUT:
// 		int (TRUE or FALSE)
...
// Clear all baby chickens from the board and reset the counter
// Used when exiting a kangaroo or wombat tunnel
// INPUT:
// 		board
//		pointer to baby_chicken_count
//		baby_chicken_total
// OUTPUT:
// 		void
void reset_baby_chickens(
...
// Check if position is in HEADLIGHTS or a CAR
// INPUT:
// 		board
// 		position
// OUTPUT:
...
int check_is_shocked(
    ...
    if (board[position.row][position.col].entity == HEADLIGHTS ||
...
// Check if player should lose the game
// INPUT:
// 		board
// 		player position
// OUTPUT:
    ...
    for (int row = 0; row < ROWS; row++) {
        for (int col = 0; col < COLS; col++) {
            ...
            if (
                !(row == player.row && col == player.col) &&
                board[row][col].baby_chicken > 0 &&
                !check_baby_chicken_connected(board, chicken, player)
            ) {
                return TRUE;
            }
        }
...
// Check if a position has a wombat tunnel
// INPUT:
// 		board
// 		position
// OUTPUT:
...
// Check if a position has a kangaroo
// INPUT:
// 		board
// 		position
// OUTPUT:
...
// Check if a position is a baby chicken
// INPUT:
// 		board
// 		position
// OUTPUT:
...
// OUTPUT:
// 		int (TRUE or FALSE)
int check_is_valid_car_dest(struct tile board[ROWS][COLS], int row, int col) {
    struct position destination;
    destination.row = row;
    destination.col = col;
...
// Check that a baby chicken is always attached to either
// another baby chicken or the player
// INPUT:
// 		board
// 		position
// 		player position
