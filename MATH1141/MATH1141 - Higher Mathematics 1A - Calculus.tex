...
\section{The definition of $\displaystyle\lim_{x\to\inf} f(x)$}
...
\section{Proving that $\displaystyle\lim_{x\to\infty} f(x) = L$ using the limit definition}
	...
	\subsection{Proofs of basic limit results}
	...
	\subsection{Rules for limits at a point}
		...
		\subsubsection{Pinching theorem (two-sided limit)}
			...
			\begin{question}{Example}
				...
				\end{align*}
				\begin{align*}
					& \text{Since } \lim_{x\to 3^+} f(x) \neq \lim_{x\to 3^-} f(x), \text{ two-sided limit does not exist at } x = 3	\\
					& \text{So, } f \text{ is not continuous at } x = 3
				\end{align*}
			\end{question}
